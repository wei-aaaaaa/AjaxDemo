...
namespace MSIT158Site.Controllers
{
    public class ApiController : Controller
    {
        ...
        {
            var memberName = _context.Members.FirstOrDefault(r => r.Name == name);
            var memberEmail = _context.Members.FirstOrDefault(r => r.Email == email);


            if (memberName != null)
            {
                return Content("此名稱已被使用, 請再試一次", "text/html", Encoding.UTF8);
            }
            else
            {
                return Content("可使用此名稱", "text/html", Encoding.UTF8);
            }

            //if (memberName != null && memberEmail != null) //代表資料庫中有相同姓名
            //{
            //    return Content("此姓名與電子郵件已被使用, 請再試一次", "text/html", Encoding.UTF8);
            //}
            ...
            //{
            //    return Content("此電子郵件已被使用, 請再試一次", "text/html", Encoding.UTF8);
            //}
            //else
            //{
            //    return Content("可使用此姓名與電子郵件", "text/html", Encoding.UTF8);
            //}
        }
